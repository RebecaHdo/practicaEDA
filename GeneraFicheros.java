...
public class GeneraFicheros {
	...
	// Tambien guarda la primera y ultima url de cada subfichero en un fichero principal llamado maestro.
	public static void dividirFichero(int bytes, String[] fich_string) throws IOException{
		...
		maestro = new FileWriter("maestro.txt");
		maestro_writer = new PrintWriter(maestro);
		
	

		int contador = 0;//Cuenta la url por la que se llega del fich_string.
		int contador_fichero= 0; //Cuenta el fichero por el que se llega creando.
		...
		while(contador < fich_string.length) {
			...
			maestro_writer.println(fich_string[contador]);	//Se guarda en el maestro la url inicial.
			...
			maestro_writer.println(fich_string[contador-1]); // Se guarda en el maestro la ultima url del subfichero.
			contador_fichero++;
		}
