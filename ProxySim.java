...
public class ProxySim {
	...
	//El argumento "maestro" es para diferenciar el fichero maestro delñ resto de ficheros.
	public static boolean leeSubfichero(String nomfich, boolean maestro) throws IOException {
		...
		if(maestro){
			TAM_ACT = fis.read(subfich);  // Lectura del maestro, su tamaño es distinto al de el resto de subficheros.
	...
	public static void main(String[] args) {
		...
		//Guarda el fichero maestro en formato bytes[] , "maestro", con su correspondeinte indice en int[],"maestroIndice".
		try {
			 leeSubfichero("maestro.txt", true);
		...
		try (BufferedReader fich = new BufferedReader(new FileReader(nomFich), 400))
		{
		   while(fich.ready()) {
			      ...
			      * Si igual a uno del maestro esa url ya es directamente peligrosa ahorrando una lectura de archivo.
			      * 
			      * Por cada par del maestro primera-ultima de cada subfichero.
