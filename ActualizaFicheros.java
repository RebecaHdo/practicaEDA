...
public class ActualizaFicheros {
		...
		public static void main(String[] args) {
			...
			in.close();
			//obtiene el número de subficheros usando el fichero maestro.
			int numeroFicheros=0;
			try {
			numeroFicheros = leeFich("maestro.txt").length/2;
			}
			catch (IOException e){
				System.out.println("Error al editar el subfichero maestro.");
				System.exit(1);
			}
			
			//Reajusta el tamaño de los ficheros y crea el maestro de nuevo.
			try {
			...
			File maestro = new File("maestro.txt");
			maestro.delete();
			try {
			crearmaestro();
			}catch (IOException e){
		...
		//la del siguiente utilizando el fichero maestro.
				
		public static int buscarSubfich(String url) throws IOException {
			int indice = -1;
			int i =2;
			String[] maestro = leeFich("maestro.txt");
		...
		public static void ajustaTamaño(int indice, int numero_ficheros) throws IOException {
			 ...
			 */
			int bytes = 102000; //Tamaño de cada subarchivo en bytes
		...
		private static void crearmaestro() throws IOException {
			...
			maestro = new FileWriter("maestro.txt", true);
			...
			//Bucle que guarda en el fichero maestro la primera y la última URL de cada subfichero.
			while(file.exists()){
